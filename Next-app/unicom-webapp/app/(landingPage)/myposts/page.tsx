...
//       const idToken = localStorage.getItem("id_token");
//       if (!idToken) return;
//       const resp = await fetch(`${API_URL}api/users/${encodeURIComponent(email)}`, {
...
//         if (post.image_url?.length && signedUrlData) {
//           const urls = post.image_url.map(u => {
//             try {
//               const p = new URL(u);
//               const cf = CLOUDFRONT_HOST + p.pathname.replace(/^\/+/, "");
//               return buildSignedImageUrl(cf, signedUrlData);
//             } catch { return u; }
//           });
//           return { ...post, image_url: urls };
//         }
//         return post;
//       });
//       setPosts(fetched);
//       Array.from(new Set(fetched.map(p => p.owner))).forEach(fetchUserInfo);
//     } catch (e) {
//       console.error(e);
//     } finally {
//       setLoading(false);
...
//       const idToken = localStorage.getItem("id_token");
//       if (!idToken) return;
//       const resp = await fetch(`${API_URL}api/myposts/semanticsearch`, {
//         method: "POST",
//         headers: {
//           "Content-Type": "application/json",
//           Authorization: `Bearer ${idToken}`,
//         },
//         body: JSON.stringify({ query: semanticSearchTerm }),
//       });
//       if (!resp.ok) return;
//       let results = (await resp.json()) as Post[];
...
//       setIsSemanticSearch(true);
//       Array.from(new Set(results.map(p => p.owner))).forEach(fetchUserInfo);
//     } catch (e) {
//       console.error(e);
//     } finally {
//       setSemanticSearching(false);
...
//   useEffect(() => {
//     if (!signedUrlData) return;
//     if (!isSemanticSearch) fetchMyPosts();
//   }, [page, limit, searchQuery, category, sortOrder, signedUrlData, isSemanticSearch]);

//   const display = isSemanticSearch ? searchResults : posts;

//   if (loading && !isSemanticSearch) return <div className="flex justify-center items-center h-screen">Loading...</div>;
...
//           <button
//             onClick={() => setIsSemanticSearch(false)}
//             className={`px-4 py-2 rounded-md ${!isSemanticSearch ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700"}`}
...
//               value={semanticSearchTerm}
//               onChange={e => setSemanticSearchTerm(e.target.value)}
//               placeholder="Describe your post to search..."
//               className="border p-2 rounded-l-md w-full max-w-md"
//             />
//             <button
//               onClick={handleSemanticSearch}
//               disabled={semanticSearching}
//               className="px-4 py-2 bg-blue-500 text-white rounded-r-md hover:bg-blue-600 disabled:bg-blue-300"
//             >{semanticSearching ? "Searching..." : "Search"}</button>
//             {searchResults.length > 0 && (
//               <button onClick={resetSemantic} className="ml-2 px-4 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400">Clear</button>
//             )}
//           </div>
//         ) : (
//           <div className="flex items-center mb-6">
...
//             />
//             <button
//               onClick={() => { setPage(1); setSearchQuery(searchTerm); }}
//               className="px-4 py-2 bg-blue-500 text-white rounded-r-md hover:bg-blue-600"
//             >Search</button>
//           </div>
//         )}
...
//         {!isSemanticSearch && (
//           <div className="flex flex-wrap gap-4 mb-6">
//             <select value={category} onChange={e => { setCategory(e.target.value); setPage(1); }} className="border p-2 rounded-md">
//               <option value="">All Categories</option>
//               <option value="SELL">Sell</option>
//               <option value="ROOMMATE">Roommate</option>
//               <option value="CARPOOL">Carpool</option>
//             </select>
//             <select value={sortOrder} onChange={e => { setSortOrder(e.target.value); setPage(1); }} className="border p-2 rounded-md">
//               <option value="desc">Newest First</option>
//               <option value="asc">Oldest First</option>
//             </select>
//           </div>
...
//             const userInfo = userInfoCache[post.owner];
//             return (
//               <div key={post._id} className="border rounded-md overflow-hidden shadow-sm bg-white cursor-pointer hover:shadow-md transition relative" onClick={() => setSelectedPost(post)}>
//                 <div className="h-48 w-full bg-gray-200">
//                   {post.image_url?.[0] ? <img src={post.image_url[0]} alt={post.title} className="object-cover h-full w-full"/> : 
//                     <div className="flex justify-center items-center h-full text-gray-500">No Image</div>}
//                 </div>
//                 <div className="p-4">
//                   <h3 className="text-lg font-bold">{post.title}</h3>
//                   <p className="text-sm text-gray-500 mb-1">{post.category}</p>
//                   <p className="text-sm text-gray-500 mb-1">Status: {post.status}</p>
//                   {/* ... category details ... */}
//                   <p className="text-xs text-gray-400 mt-2">Posted by <a href={`mailto:${post.owner}`} className="text-blue-500 hover:underline">{post.owner}</a></p>
//                   {userInfo && <div className="text-xs text-gray-500 mt-2 space-y-1"><div><strong>Name:</strong> {userInfo.name}</div><div><strong>College:</strong> {userInfo.college}</div><div><strong>Department:</strong> {userInfo.department}</div></div>}
...
//         {!isSemanticSearch && <div className="flex justify-between items-center mt-8"><button onClick={() => page>1 && setPage(p=>p-1)} disabled={page===1} className="px-4 py-2 bg-gray-100 rounded hover:bg-gray-200 disabled:opacity-50">Previous</button><p className="text-gray-700">Page {page} of {Math.ceil(total/limit)}</p><button onClick={() => page*limit<total && setPage(p=>p+1)} disabled={page*limit>=total} className="px-4 py-2 bg-gray-100 rounded hover:bg-gray-200 disabled:opacity-50">Next</button></div>}
//       </section>

//       <AnimatePresence>{selectedPost && (<motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 300, damping: 30 }} className="fixed top-0 right-0 w-full md:w-1/2 h-full bg-white shadow-2xl z-50 overflow-auto"><div className="p-6">{/* modal content */}</div></motion.div>)}</AnimatePresence>
//     </div>
//   );
// }
